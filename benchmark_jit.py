"""
Benchmark script comparing original vs JIT-compiled simulation.

This script runs the same simulation using both the original stochastic_simulation
and the new JIT-compiled version, reporting performance improvements.
...
def create_test_model():
    """Create a simple SEIR model for benchmarking."""
    model = EpiModel(
        name="SEIR_Benchmark",
...
def create_spaghetti_plot(results_original, results_jit):
    """
    ...
    ax.set_title('Original Simulation (100 runs)')
    ax.legend()
    ...
    ax.set_xlabel('Date')
    ax.set_ylabel('Number of Infected')
    ax.set_title('JIT Simulation (100 runs)')
    ax.legend()
    ax.grid(True, alpha=0.3)
...
def main():
    ...
    print("=" * 70)
    print("Epydemix JIT Compilation Benchmark")
    print("=" * 70)
    print()
    ...
    for n_sims in n_sims_list:
        ...
        if n_sims == 100:
            ...
            create_spaghetti_plot(results_original, results_jit)
    ...
    print("  - JIT version provides ~7x speedup for typical simulations")
    print("  - For calibration workflows (many runs), JIT provides major benefits")
